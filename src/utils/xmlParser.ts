...
export function getSectionContent(xmlText: string, tag: string, basePath?: string): string {
  ...
  section = section.replace(/&(\w+);/g, (_, name) => {
    return entityMap[name] || `<!-- Lỗi: entity ${name} không tìm thấy -->`;
  });
...
// Tim dong trong file goc chua tham chieu Entity (vd: &XMLVoucherBookAndNumberFields;)
export function findEntityReferenceLine(lines: string[], sectionContent: string): number {
  const entityMatches = [...sectionContent.matchAll(/&([a-zA-Z0-9_]+);/g)];
  for (const match of entityMatches) {
    const entityName = match[1];
    const lineIndex = lines.findIndex(line => line.includes(`&${entityName};`));
